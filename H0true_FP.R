...
# p-values where df based on trial - False positives of test when random effect exists in 1 ppt
nSim<-1000
n<-10       #total ppt
...
for (runs in c(1:nSim)){
  ...
  cond2ppt_m<-rnorm(n,5,1)
  
  cond1DV <- numeric(0)
  cond2DV <- numeric(0)
  cond1DV <- c(cond1DV, rnorm(ntrial, cond1ppt_m[i], 1)) #random effect for 1st ppt
  cond2DV <- c(cond2DV, rnorm(ntrial, cond2ppt_m[i], 1))
  for (i in c(2:n)){
    cond1DV <- c(cond1DV, rnorm(ntrial, cond1ppt_m[i], 2)) # all others are sampled from the same distribution
    cond2DV <- c(cond2DV, rnorm(ntrial, cond1ppt_m[i], 2))
  }
  data <- data.frame(
