...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# Luise Gootjes-Dreesbach, 25th Nov 2016
# Modelling from paper: 
# Verghese, P. (2001). Visual search and attention: A signal detection theory approach. Neuron, 31(4), 523-535.

# # # # # # # # # # # # PARAMETERS
numit<-3  # Number of items in the search display (Default: 8 or 32 for last two rows of fig. 1, respectively)
...
sim<-100000
d<-rnorm(sim,mu1,sd1) # Distractor samples
t<-rnorm(sim,mu2,sd2) # Target samples
p<-t-d
p<-length(p[p>0])/sim # Percentage of samples in which target wins
text(11, 0.9, paste("p=",p))

# # # # # # # # # # # # Figure 2: Monitoring several items per display
